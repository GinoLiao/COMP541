...
%
%Our calibration mechanism is a brute force search that calculates the \gls{DTW} distance of one attempt time series to another for the same gesture. The calibrated time series selected is the attempt with the smallest average distance to all of the other attempts. This approach is non-scalable ($O(N^{2})$). However, scalability is not the focus of this project because we can make calibration a one-time event at the beginning and choose not to update the history for the purposes of this experiment.
%
%\subsection{Threshold Setting}
%
%The threshold for rejecting or accepting a user can be set using a variety of techniques. For this project, we use a simple threshold ($D_{thres}$) based on the calibration data above and is described by \autoref{eq:thres}. Each attempt from calibration has an average \gls{DTW} distance from all of the other calibration attempts. Choosing the minimum recorded \gls{DTW} distance yields  
%
%\begin{equation}
%\begin{split}
%D_{thres} = m \cdot d_{p} \pm 5 \\ 
%\end{split}
%\label{eq:thres}
%\end{equation}
%
